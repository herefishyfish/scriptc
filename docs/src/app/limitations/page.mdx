...
- The Android target is synchronous and UI-thread-only. Constructors and
  instance calls resolve through NativeScript-generated Android metadata, with
  generic JNI dispatch for object, string, boolean, number, and zero-argument
  void callback parameters. Static members, fields, arrays, richer callback
  signatures, async/promise event-loop integration, and background-thread JNI
  attachment are not implemented yet.
