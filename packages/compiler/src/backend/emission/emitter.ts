...
export class CEmitter {
  ...
  emit(): string {
    ...
    const out: string[] = [
      ...
      ...(this.options.host === "android" ? [`#include <jni.h>`] : []),
      ...(this.options.host === "android" ? [`#include "scr_android.h"`] : []),
      `#include <math.h>`,
      `#include <stdio.h>`,
      `#include <stdlib.h>`,
      ``,
    ...
    if (refGlobals.length > 0 || fnValueProps.length > 0) {
      ...
      for (const g of refGlobals) {
        ...
        out.push(`  ${mangleGlobal(g.id)} = NULL;`);
      }
    ...
    const resetAndroidGlobals = globals.flatMap((g) => {
      if (isRefCounted(g.type)) return [];
      switch (g.type.kind) {
        case "bool":
          return [`  ${mangleGlobal(g.id)} = false;`];
        case "f64":
        case "procStream":
          return [`  ${mangleGlobal(g.id)} = 0.0;`];
        default:
          return [];
      }
    });
    ...
    if (this.options.host === "android") {
      ...
      out.push(
        ...
        `Java_org_scriptc_runtime_ScriptcRuntime_start(JNIEnv *env, jclass cls, jobject activity) {`,
        `  (void)cls;`,
        `  if (sc_android_started) return 0;`,
        `  scr_init();`,
        ...this.errorVtStampLines(),
        ...emitterVtStampLines(this),
        ...streamVtStampLines(this),
        `  if (!scr_android_init(env, activity)) {`,
        ...
        `Java_org_scriptc_runtime_ScriptcRuntime_shutdown(JNIEnv *env, jclass cls) {`,
        `  (void)env;`,
        `  (void)cls;`,
        `  if (!sc_android_started) return;`,
        releaseGlobals,
        ...resetAndroidGlobals,
        `  scr_android_shutdown();`,
        `  sc_android_started = false;`,
        `}`,
        ``,
      );
      return out.join("\n");
